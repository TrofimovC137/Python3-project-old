...
def useless_checks(matr):
    ...
    for i in range(len(matr.matrix[0])):
        if (0 in matr.get_Zi(i+1))==False or (1 in matr.get_Zi(i+1))==False:
            checks.append(i+1)
    return checks
def choose_the_best_check(matr):
    C=[]
...
def choose_the_best_check_group(matr_list):
...
def main(matrix):
    ...
    while (len(state)!=0):
        print('итерация №',k)
        k+=1
        print('имеются следующие невыделенные состояния системы')
        for i in state:
            i.print_matrix()
            print('_______________________________')
        if len(state)==1:
            checks.append(choose_the_best_check(state[0]))
            print('наилучшая проверка  - Z',checks[-1])
            state=[state[0].S0(checks[-1]),state[0].S1(checks[-1])]
        else:
            checks.append(choose_the_best_check_group(state))
